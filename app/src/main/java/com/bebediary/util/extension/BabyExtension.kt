...
fun Baby.eventDateToCalendarText(now: Calendar = Calendar.getInstance()): String {
    return if (isPregnant) this.dueDateToCalendarText(now) else this.birthdayToCalendarText(now)
}
...
fun Baby.dueDateToText(): String {
    ...
    // 날짜 차이
    val diffDays =
        TimeUnit.MILLISECONDS.toDays(todayCalendar.timeInMillis - pregnantCalendar.timeInMillis)
...
fun Baby.dueDateToCalendarText(now: Calendar = Calendar.getInstance()): String {
    ...
    return if (diffDays < 0) "" else "'$month'주차 '$days'일"
}

fun Baby.birthdayToCalendarText(now: Calendar = Calendar.getInstance()): String {
    ...
    // 태어난 후 오늘까지 몇일이 지났는지
    val diffDay = TimeUnit.MILLISECONDS.toDays(todayCalendar.timeInMillis - date.time)
    ...
    return if (diffDay < 0) "" else "만 ${age}세 ${month}개월"
}